...
func Test_maxProbabilityPV1(t *testing.T) {
	type args struct {
		n          int
		edges      [][]int
		succProb   []float64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := maxProbabilityPV1(tt.args.n, tt.args.edges, tt.args.succProb, tt.args.start_node, tt.args.end_node); got != tt.want {
				t.Errorf("maxProbabilityPV1() = %v, want %v", got, tt.want)
			}
		})
	}
}
