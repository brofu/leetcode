...
func Test_accountsMergeV2(t *testing.T) {
	type args struct {
		accounts [][]string
	}
	tests := []struct {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			if got := accountsMergeV2(tt.args.accounts); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("accountsMergeV2() = %v, want %v", got, tt.want)
			}
		})
	}
}
