...
func Test_combinationSum2V3(t *testing.T) {
	type args struct {
		candidates []int
		target     int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := combinationSum2V3(tt.args.candidates, tt.args.target); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("combinationSum2V3() = %v, want %v", got, tt.want)
			}
		})
	}
}
