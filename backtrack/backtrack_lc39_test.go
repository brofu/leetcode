...
func Test_combinationSumPV3(t *testing.T) {
	type args struct {
		candidates []int
		target     int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := combinationSumPV3(tt.args.candidates, tt.args.target); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("combinationSumPV3() = %v, want %v", got, tt.want)
			}
		})
	}
}
