...
func Test_subsetsWithDupPV1(t *testing.T) {
	type args struct {
		nums []int
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := subsetsWithDupPV1(tt.args.nums); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("subsetsWithDupPV1() = %v, want %v", got, tt.want)
			}
		})
	}
}
