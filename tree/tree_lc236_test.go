...
func Test_lowestCommonAncestorPV2(t *testing.T) {
	type args struct {
		input []int
		root  *TreeNode
		p     *TreeNode
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.args.root = GenerateBinaryTreeFromSlice(tt.args.input, 0)
			if got := lowestCommonAncestorPV2(tt.args.root, tt.args.p, tt.args.q); !reflect.DeepEqual(got.Val, tt.want) {
				t.Errorf("lowestCommonAncestorPV2() = %v, want %v", got, tt.want)
			}
		})
	}
}

func Test_lowestCommonAncestorPV3(t *testing.T) {
	type args struct {
		root []int
		p    *TreeNode
		q    *TreeNode
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tree := GenerateBinaryTreeFromSlice(tt.args.root, 0)
			if got := lowestCommonAncestorPV3(tree, tt.args.p, tt.args.q); !reflect.DeepEqual(got.Val, tt.want) {
				t.Errorf("case: %s, lowestCommonAncestor() = %v, want %v", tt.name, got.Val, tt.want)
			}
		})
	}
