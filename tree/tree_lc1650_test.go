...
func Test_lowestCommonAncestorIIIV2PV1(t *testing.T) {
	type args struct {
		root []int
		p, q int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			q := GetNodeWithParentFromTree(tree, tt.args.q)
			wantNode := GetNodeWithParentFromTree(tree, tt.want)
			if got := lowestCommonAncestorIIIV2PV1(p, q); !reflect.DeepEqual(got, wantNode) {
				t.Errorf("lowestCommonAncestorIIIV2PV1() = %v, want %v", got, tt.want)
			}
		})
	}
}
