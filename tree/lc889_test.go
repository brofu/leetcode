...
func Test_constructFromPrePostV2(t *testing.T) {
	type args struct {
		preorder  []int
		postorder []int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := constructFromPrePostV2(tt.args.preorder, tt.args.postorder)
			gotSlice := GenerateSliceFromTreeNode(got)
			if !reflect.DeepEqual(gotSlice, tt.want) {
				t.Errorf("constructFromPrePostV2() = %v, want %v", gotSlice, tt.want)
			}
		})
	}
}
